...
void MNNSamplerC4Bilinear_RVV(const unsigned char* source, unsigned char* dest, MNN::CV::Point* points, size_t sta,
    ...
    if (vl < 4) {
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            for (size_t c = 0; c < 4; ++c) {
                dest[4 * i + c] = bilinearPixel(row0[4 * x0 + c], row0[4 * x1 + c], row1[4 * x0 + c],
                                                row1[4 * x1 + c], xF, yF);
            }
            currentY += dy;
            currentX += dx;
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        const vuint32m2_t c00 = __riscv_vzext_vf4_u32m2(__riscv_vle8_v_u8mf2(source + (size_t)y0 * yStride + 4 * (size_t)x0, vl), vl);
        const vuint32m2_t c01 = __riscv_vzext_vf4_u32m2(__riscv_vle8_v_u8mf2(source + (size_t)y0 * yStride + 4 * (size_t)x1, vl), vl);
        const vuint32m2_t c10 = __riscv_vzext_vf4_u32m2(__riscv_vle8_v_u8mf2(source + (size_t)y1 * yStride + 4 * (size_t)x0, vl), vl);
        const vuint32m2_t c11 = __riscv_vzext_vf4_u32m2(__riscv_vle8_v_u8mf2(source + (size_t)y1 * yStride + 4 * (size_t)x1, vl), vl);
        ...
        value = __riscv_vfadd_vv_f32m2(
            value, __riscv_vfmul_vf_f32m2(__riscv_vfcvt_f_xu_v_f32m2(c01, vl), w1, vl), vl);
        value = __riscv_vfadd_vv_f32m2(
            value, __riscv_vfmul_vf_f32m2(__riscv_vfcvt_f_xu_v_f32m2(c10, vl), w2, vl), vl);
        value = __riscv_vfadd_vv_f32m2(
            value, __riscv_vfmul_vf_f32m2(__riscv_vfcvt_f_xu_v_f32m2(c11, vl), w3, vl), vl);
