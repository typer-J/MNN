...
#define MNN_BITCOPY_TRAITS(NAME, TYPE, SEW, VTYPE, LMUL, VECTOR_TYPE)                              \
    ...
    struct NAME<TYPE> {                                                                            \
        ...
        static inline size_t setvl(size_t avl) {                                                   \
            return __riscv_vsetvl_e##SEW##LMUL(avl);                                               \
        }                                                                                          \
        static inline Vec broadcast(TYPE value, size_t vl) {                                       \
            return __riscv_vmv_v_x_##VTYPE##LMUL(value, vl);                                       \
        }                                                                                          \
...
// sides needs the narrow form. Instrumenting the full test suite showed how
// much that split matters: 77.5% of all calls are stride=2/ds=1 (the m4 side),
// 19.3% are stride=4/ds=2 (the m2 side, where m4 regresses to 0.75x and m8 to
// 0.42x).
template <typename T>
