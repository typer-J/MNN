...
void MNNC3ToFloatRGBA_RVV(const unsigned char* source, float* dest, const float* mean, const float* normal,
                          size_t count) {
    size_t i = 0;
    while (i < count) {
        ...
        for (size_t c = 0; c < 3; ++c) {
            ...
            __riscv_vsse32_v_f32m8(dest + 4 * i + c, 4 * sizeof(float), value, vl);
        }
        const vfloat32m8_t zero = __riscv_vfmv_v_f_f32m8(0.0f, vl);
        __riscv_vsse32_v_f32m8(dest + 4 * i + 3, 4 * sizeof(float), zero, vl);
        i += vl;
    }
}
