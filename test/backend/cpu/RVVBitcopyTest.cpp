...
bool checkKernel(const BitcopyKernels& kernel, int size, int stride, int ds, size_t& cases) {
    ...
    // The generic arm is only expected to match inside the documented contract.
    // Outside it the generic Fast kernels ignore ds altogether, which is a
    // pre-existing defect that this test records rather than enshrines.
