...
布局探索先核对特殊形状的实际调用分支：例如 E=1 可能已经绕过通用 A-pack，不能把新增向量化的收益归因于消除了原本不存在的打包。
预打包方案应同时对比原始标量、当前生产 kernel 和不打包的向量化候选；仅在标量 TU 禁用自动向量化，不能顺带削弱生产 RVV 对照。
分别测量打包、稳态算子链和首次完整调用，计入布局边界转换；权重复用回本点必须相对最佳直接路径计算，并区分解析推算与真实复用实测。
同算法 P4/P8/P16 原型比较可筛选布局，但不能替代生产 GEMM、图调度、缓存干扰和模型质量验收。
