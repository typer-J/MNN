...
全库适配计划先建立“算子/函数签名 → 编译目标 → 注册字段或导出符号 → Execution 调用点 → 验证证据”清单。
文件数量、同名旧接口和其他分支的提交不能代替当前版本的有效覆盖率；现有分支还须检查补丁重叠、
接口漂移和目标编译可用性。全量盘点后按热点推进，pack 契约随首批 kernel 一起验收，不能等待所有冷门算子重写完才检查布局。
