...
### 1.5 移植型优化：先建「已完成集合」，再谈缺口

目标架构仍走标量路径时，第一件事不是读源码找缺口，而是**先建立已完成集合**：
同时检查任务表、当前分支、上游提交和活跃功能分支，把候选分成四类——
「已合入」「分支已完成」「尚未开始」「已有实现但性能回退」。两条硬要求：

- **不要仅凭当前分支的文件名判断缺失**。目录下一个文件都没有，不等于这块没做过：
  实现可能在别的功能分支上，也可能已经合入上游。
- **不要把性能低于标量的 SIMD 内核当作已完成**。「有 `_RVV` 后缀」只证明实现覆盖，
  不是收益证据。这类内核要在「已有实现但性能回退」里单列——它们的优先级高于"尚未开始"的项，
  因为已经付出的验证成本没有换来收益，而且它们的缺陷模式会误导后续选型。

缺口清单出来后，选型优先级是硬的：**成熟 SIMD 实现优先迁移**。当目标架构还在走标量时，
优先挑已有 NEON / AVX / 汇编实现的函数——先复用它的数学拆分、循环展开、边界语义和测试，
再做架构特有调优。没有可对照实现、需要重新设计算法的项目**后排**：
收益不确定，而且没有现成的正确性参照物，验证成本远高于移植。
