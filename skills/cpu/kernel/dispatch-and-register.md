...
还要核对**完整签名与链接名**：架构文件中的同名 C++ 函数不会替换契约头声明的 C 符号，旧参数列表也可能只是无人调用的重载。
用目标产物的符号表或 IR 与调用点逐项对照。保留通用定义时，优先使用独立 `_RVV` 等 helper 并在能力门内显式注册；
仅补 `extern "C"` 可能制造重复定义。直接全局替换的函数还要单独检查 ISA 能力门，不能由函数表受保护推断所有调用都安全。

函数表集成测试必须先完成实际 Backend 的延迟初始化，例如先执行一个 CPU 图再检查表；
不能把未初始化的空指针当作“不适用”而静默通过。硬件 `supportXxx` 在关闭 ISA 构建开关时仍可为真，
因此测试应同时取得明确的构建标记与运行时能力位，分别断言优化注册和通用回退。
内部符号在共享库中可能隐藏；直接函数表测试应限定到可链接这些符号的静态 CPU 测试路径，
保留共享库和其他后端的正常算子测试，避免为了测试而扩大生产 API 导出。
