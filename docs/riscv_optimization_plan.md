...
| 首批生产代码候选 | `fe7f86c6`，工作分支 `codex/riscv-sg2044-first-pass`；已收到四函数目标微基准通过摘要，原始性能分析与完整 MNN 验收待完成 |
...
| SG2044 正确性与性能结果 | 四函数1320配置正确性及采样已通过；原始结果包待核验，尚无加速比和完整 MNN 派发/模型结论 |
...
|---|---|---|---|
| C4 pack/unpack、Scale、PReLU 接入 | linkage 问题待修 | 四项接线已实现，目标静态编译及签名检查通过；四函数标量对照已执行 | 已收到1320配置正确性与三性能进程通过摘要；速度及完整库派发待核验 |
...
| 目标环境预检（用户回传日志） | GCC 14.4.0，riscv64 target；标准库/线程程序通过；RVV FP32 探针通过，VLEN=128，hwprobe 报告 V | 原始结果包待核验，不外推其他机器或完整 MNN 派发 |
| 四函数目标正确性（v2 回传摘要） | 1320个参数配置，failed=0，无计时 | 原标量与生产 RVV 函数直接对照，尚未检查完整 MNN 调用链 |
| 三个独立目标性能进程（v2 回传摘要） | 每个1320配置、failed=0、18480条计时采样；总入口passed | 只证明数据已采集；尚未收到逐case CSV，不能据passed推断加速 |
| 完整 MNN 构建、op/模型与实际派发验证 | 显式跳过 | integration=explicitly_skipped；仍须独立完成 |

本机语义模拟使用相同生产函数体和原标量函数体，覆盖尾通道、非连续 stride、原位 Scale/PReLU、
正负零、subnormal、Inf、NaN 与保护区。模拟不能替代目标执行；目前已收到上述首次目标成功摘要，原始数据仍待核验。
...
按 runner 的执行顺序判断，微基准已完成构建并尝试运行，原始 build/run 日志与产物 hash 尚待结果包核验。
...
下一步先核验结果包校验和、源码/编译器/对象身份和逐case中位数/MAD，再判断四项接入的适用范围、退化和后续调优。
...
构建与运行的日志、退出码和原始数据保留在新结果目录；当前函数级阶段已回传成功摘要，完整库阶段仍待执行。
