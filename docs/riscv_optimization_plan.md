...
| 首批生产代码候选 | `fe7f86c6`，工作分支 `codex/riscv-sg2044-first-pass`；尚未通过 SG2044 板端验收 |
| 本文源码结论 | 针对以上合并基线的静态审计，固定版本链接用于避免后续修改混淆现状 |
| 第一批候选工作 | C4 pack/unpack、Scale、PReLU 的 RVV 接线修复已实现；目标静态编译及函数指针签名检查通过，标量对照与微基准见 `benchmark/rvv_scalar_compare/`，板端待验 |
| SG2044 正确性与性能结果 | 本文建立时尚未取得；后续候选必须上板验证，不能据静态审计宣称加速完成 |
...
|---|---|---|---|
| C4 pack/unpack、Scale、PReLU 接入 | linkage 问题待修 | 四项接线已实现，目标静态编译及签名检查通过；标量对照代码已准备 | 未取得 |
...
| 目标环境预检（用户回传日志） | GCC 14.4.0，riscv64 target；标准库/线程程序通过；RVV FP32 探针通过，VLEN=128，hwprobe 报告 V | 只证明预检；尚未收到原始结果包，不外推芯片规格或四个生产函数通过 |
| 四个生产函数的真机正确性与性能、完整 MNN 构建及 op/模型回归 | 尚未完成 | 微基准编译通过后仍可能在启动或参数检查处退出；尚无真机加速比 |
...
四个生产内核不需为此改动；尚无这次修复后的板端函数正确性或性能结果。
