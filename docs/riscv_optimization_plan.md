...
更新日期：2026-09-10。原始目标硬件：SG2044；目标编译器：GCC 14。

### 2026-09-10 进展

直接 FP32 E=1 GEMV 已完成生产候选并提交 [PR #4847](https://github.com/alibaba/MNN/pull/4847)，提交 `976b950e6`。
非转置 B 沿用 raw ABI，通过能力门注册 RVV；转置 B 与关闭 RVV 的回退已逐位验证。
GCC 14.3.1、VLEN=128 下，实际新派发范围 H>1 的 32 个配置默认编译几何平均加速 1.864×，31 个配置稳定提升超过 5%，无稳定退化超过 5%。
RVV ON/OFF × 1/4 线程 × 精度参数 1/2 的完整 MatMul 矩阵共 16 项测试通过，每次另验证 2048 个线程、尾部和特殊值组合。
此前 P4/P8/P16 布局试验保存在 `feature/rvv-layout-experiment`；后续优先评估转置 GEMV，再做高复用权重 P16 缓存与局部 C16 算子链，不直接替换全局 C4。
这些是 kernel 和完整算子测试证据，尚不代表模型端到端收益；PR 仍待上游审核合入。

在用户提供的两台 RISC-V 服务器中选择空闲的第一台，使用隔离源码继续验证。
本轮测得 VLEN=128、系统 GCC 14.3.1；与此前 GCC 14.4.0 的回传结果分组记录。
Scale/PReLU 的连续 C4 访存、小平面优化及运行时注册已提交
[PR #4846](https://github.com/alibaba/MNN/pull/4846)，基于 upstream/master `fe15068b`。
原始标量对照、三进程采样与完整静态 MNN 的 RVV ON/OFF 集成矩阵均已通过；仍有极小张量性能退化，详见 PR。
已有 Pack 优化留在 [PR #4840](https://github.com/alibaba/MNN/pull/4840)，不重复提交。
后续继续按下述顺序核对剩余热点和派发、pack、执行器、Attention/KV、线程内存与精度覆盖。
以下审计表保留原始固定基线，不表示所有已提交 PR 都已合入 upstream，也不表示模型端到端验收已完成。
